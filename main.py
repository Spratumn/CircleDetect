...
def main(cfg):
    ...
    for epoch in range(start_epoch + 1, cfg.NUM_EPOCHS + 1):
        mark = epoch if cfg.SAVE_ALL else 'last'
        # run train, get
        ...
        if cfg.VAL_INTERVALS > 0 and epoch % cfg.VAL_INTERVALS == 0:
            save_model(os.path.join(cfg.WEIGHTS_DIR, 'model_{}.pth'.format(mark)),
                       epoch, model, optimizer)
            with torch.no_grad():
                log_dict_val, preds = trainer.val(epoch, eval_loader)
...
if __name__ == '__main__':
    cfg = Config()
    # main(cfg)
    train_loader = DataLoader(CircleDataset(cfg, 'test'),
                              batch_size=cfg.BATCH_SIZE,
                              shuffle=True,
                              num_workers=cfg.NUM_WORKERS,
                              pin_memory=True,
                              drop_last=True)
    ret = train_loader.dataset.__getitem__(1)
