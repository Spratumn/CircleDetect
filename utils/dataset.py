...
from utils.label_process import get_gaussian_radius, gaussian2D, draw_dense_wh, draw_gaussian, draw_background_gaussian


class TrainCircleDataset(Dataset):
    """
    ...
    def __init__(self, cfg):
        super(TrainCircleDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        for k in range(len(bboxes)):
            ...
            if h > 0 and w > 0:
                ...
                heatmap = hm[1:]
                draw_dense_wh(dense_wh, heatmap.max(axis=0), ct_int, wh[k], gaussian, radius)
                # 3.3.5 draw gaussian on hm[class_id] with center and radius
                draw_gaussian(hm[class_id], ct_int, gaussian, radius)
                draw_background_gaussian(hm[0], ct_int, gaussian, radius)
                gt_det.append([ct[0] - w / 2, ct[1] - h / 2,
                               ct[0] + w / 2, ct[1] + h / 2, 1, 0])

        hm_a = hm[1:].max(axis=0, keepdims=True)
...
class ValCircleDataset:
    """
    load images and labels
    labels: boxes:[[x1,y1],[x2,y2]]
    """
    def __init__(self):
        # read csv as data index
        # for circle detection
        anns = np.load('data/dataset/augment data/eval_anns.npy', allow_pickle=True).item()
        self.image_paths = anns['image_paths']
        self.bboxs = anns['bboxs']  # [x1,y1,x2,y2]
        self.num_samples = len(self.image_paths)
        print('Loaded {} samples'.format(self.num_samples))
